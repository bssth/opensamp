...
///
/// ---------------------------------------------------------------------
/// MODIFIED BY THE OPENSAMP PROJECT, 2026-08-08.
///
/// OpenSAMP exercises the GPL option above and distributes this file under
/// GPL-3.0. Changes from the sampraknet baseline (e891acf):
///
///   * added invoke_rpc_handler_safe(), an SEH trampoline, and routed both
///     RPC dispatch call sites in HandleRPCPacket() through it, so a fault
///     inside a handler is logged instead of killing the game;
///   * downgraded the UNDEFINED_RPC_INDEX and missing-node RakAsserts to
///     silent returns, because servers stream many RPCs a client is not
///     required to register;
///   * removed the per-RPC trace that used to sit in HandleRPCPacket.
///
/// See native/vendor/sampraknet/OPENSAMP-NOTICE.md.
/// ---------------------------------------------------------------------
...
// Defined in sampraknet_bridge.cpp — shared global log that routes into chat.
extern void Log(const char* fmt, ...);

// SEH trampoline. MSVC refuses to compile __try inside functions that need
// C++ unwinding, and HandleRPCPacket constructs BitStream locally. Defer the
// handler call through this C-style shim so our try/except sits in a frame
// with no destructors.
typedef void (*RpcStaticFn)(struct RPCParameters*);

static int invoke_rpc_handler_safe(RpcStaticFn fn, struct RPCParameters* p,
                                   unsigned rpc_id, unsigned bits)
{
    __try
    {
        fn(p);
    ...
    {
        Log("[RPC] !! exception in handler for id=%u bits=%u", rpc_id, bits);
        return 0;
    }
}
...
bool RakPeer::HandleRPCPacket( const char *data, int length, PlayerID playerId )
{
	...
	// RPC tracing used to happen here while we were bringing up the protocol.
	// Removed — at 30Hz sync rates it drowned the log and stalled the game
	// through the disk-write path. Re-enable locally when investigating a
	// specific RPC. RakNet stores the RPC id inside the pointer value itself.
	// const unsigned rpc_id = (unsigned)(uintptr_t)uniqueIdentifier;

	if (rpcIndex==UNDEFINED_RPC_INDEX)
	{
		// Unregistered function — SAMP server streams many RPCs we don't care
		// about in the early phases (spawn info, weapons, clothes, world
		// stream, etc.). Silently drop instead of asserting.
		return false;
	}
	...
	if (node==0)
	{
		// Index resolved but node missing — treat the same way.
		return false;
	}
	...
	{
		invoke_rpc_handler_safe(node->staticFunctionPointer, &rpcParms,
		                        (unsigned)(uintptr_t)uniqueIdentifier, 0);
	}
	else
	{
		...
		// Call the function callback — wrapped in SEH so a buggy handler
		// drops this one packet instead of taking the whole game down.
		rpcParms.input=userData;
		invoke_rpc_handler_safe(node->staticFunctionPointer, &rpcParms,
		                        (unsigned)(uintptr_t)uniqueIdentifier,
		                        (unsigned)rpcParms.numberOfBitsOfData);
